 ...
 */
 #include "main.h"
 #include "temp.h"
lv_obj_t* create_temperature_bar(lv_obj_t *parent, int32_t min_temp, int32_t max_temp, int32_t init_temp, lv_coord_t x, lv_coord_t y)
{
    // 创建样式
    ...
    lv_obj_set_size(container, 60, 240); // 增加高度以容纳温度文本
    lv_obj_set_pos(container, x, y);
    lv_obj_set_style_bg_opa(container, LV_OPA_TRANSP, 0); // 透明背景
    lv_obj_clear_flag(container, LV_OBJ_FLAG_SCROLLABLE); // 不可滚动
    ...
    lv_obj_center(bar);
    lv_obj_align(bar, LV_ALIGN_BOTTOM_MID, 0, -10); // 底部对齐，留出上方空间显示文本
    ...
    lv_obj_align(temp_label, LV_ALIGN_TOP_MID, 0, 5); // 位于温度条上方
    lv_obj_set_style_text_font(temp_label, &lv_font_montserrat_16, 0); // 使用合适的字体
 ...
 */
void lv_example_bar_in_tab3(lv_obj_t *tab3)
{
    // 创建温度条（位置可根据需要调整）
    lv_obj_t *temp_bar = create_temperature_bar(tab3, -20, 40, 25, 100, 50);
