 ...
 * @return 弧形量表对象指针
 */
static lv_obj_t *s_prev_arc = NULL;     // 静态变量追踪上一个圆弧
static lv_obj_t *s_prev_label = NULL;   // 静态变量追踪上一个标签
...
void create_humidity_arc(lv_obj_t *parent, lv_coord_t x, lv_coord_t y, uint8_t init_value)
{
    // 第一步：销毁上一次创建的组件
    if (s_prev_arc != NULL) {
        lv_obj_del(s_prev_arc);       // 删除圆弧对象
        s_prev_arc = NULL;
    }
    if (s_prev_label != NULL) {
        lv_obj_del(s_prev_label);     // 删除标签对象
        s_prev_label = NULL;
    }

    // 第二步：创建新的弧形量表
    lv_obj_t *new_arc = lv_arc_create(parent ? parent : lv_scr_act()); // 父容器为空时默认创建在屏幕上
    lv_obj_set_size(new_arc, 90, 90);
    lv_obj_set_pos(new_arc, x, y);
    lv_arc_set_rotation(new_arc, 135);          // 旋转起始角度
    lv_arc_set_bg_angles(new_arc, 0, 270);       // 背景弧度范围
    lv_arc_set_value(new_arc, init_value);       // 初始值

    // 创建新的数值显示标签
    lv_obj_t *new_label = lv_label_create(parent ? parent : lv_scr_act());
    lv_label_set_text(new_label, "0%");
    lv_obj_set_style_text_font(new_label, &font20, 0);
    lv_obj_set_style_text_align(new_label, LV_TEXT_ALIGN_CENTER, 0);
    lv_obj_align(new_label, LV_ALIGN_TOP_LEFT, 40, 130); // 保持原有布局参数

    // 绑定事件：值变化时更新标签（使用新标签）
    lv_obj_add_event_cb(new_arc, value_changed_event_cb, LV_EVENT_VALUE_CHANGED, new_label);
    lv_event_send(new_arc, LV_EVENT_VALUE_CHANGED, NULL); // 初始化标签显示

    // 更新静态变量追踪当前组件
    s_prev_arc = new_arc;
    s_prev_label = new_label;
}
